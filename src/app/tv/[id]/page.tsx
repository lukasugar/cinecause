...
import { tmdbTvToMediaItem } from '@/lib/adapters';
...
export async function generateMetadata({ params }: TvPageProps) {
  ...
  try {
    const tv = await getTvById(parseInt(id));
    return {
      title: `${tv.name} - CineCause`,
      description: tv.overview,
    };
  } catch {
    return {
      title: 'TV Show Not Found - CineCause',
    };
  }
}
...
export default async function TvPage({ params }: TvPageProps) {
  ...
  let tvDetail;
  try {
    tvDetail = await getTvById(tmdbId);
  } catch {
    notFound();
  }

  const item = tmdbTvToMediaItem(tvDetail);
      ...
      <div className="grid gap-8 md:grid-cols-[300px_1fr]">
        ...
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-4 text-[var(--text-muted)]">
            ...
            {item.runtime && <span>{item.runtime} min/ep</span>}
          </div>
          ...
          {mediaStats && mediaStats.donationCount && mediaStats.donationCount > 0 && (
            <div className="mt-4 rounded-lg border border-[var(--success)]/60 bg-[rgba(10,50,35,0.32)] p-4">
              <p className="font-medium text-[var(--success)]">
                This show has inspired {formatCents(mediaStats.totalDonationsCents)} in donations
              </p>
          ...
          {item.director && (
            <p className="mt-4 text-sm text-[var(--text-muted)]">
              <span className="font-medium text-[var(--text-strong)]">Creator:</span> {item.director}
            </p>
          )}
